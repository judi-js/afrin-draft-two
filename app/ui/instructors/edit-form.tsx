...
import { InstructorFormState, updateInstructor } from '@/app/lib/actions';
...
export default function EditSessionForm({
  ...
  const [state, formAction, isPending] = useActionState(updateInstructorWithId, initialState);
    ...
    <form action={formAction}>
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button type="submit" disabled={isPending} aria-disabled={isPending}>
          {isPending ? 'جاري التعديل...' : 'تعديل مدرس'}
        </Button>
      </div>
    </form>
  );
